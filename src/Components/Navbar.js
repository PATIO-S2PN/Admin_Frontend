...
const Navbar = () => {
  ...
  return (
    <div className={`flex bg-gray-50 h-[100vh] p-5 pt-8 ${open ? 'w-72' : 'w-20'} duration-300 relative flex-col `} >
      <BsArrowLeftShort
        className={`absolute mt-10 cursor-pointer rounded-full bg-blue-100 p-1 text-3xl text-black border border-black top-9 -right-3 ${!open && 'rotate-180'}`}
  ...
  {Menu.map((menu, index) => (
    <React.Fragment key={index}>
      <li
        className={`flex items-center gap-x-4 cursor-pointer p-2.5 text-sm text-black hover:bg-gray-300 rounded-md ${menu.spacing ? 'mt-3' : 'mt-2'}`}
